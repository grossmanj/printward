...
test('SQL order context filters sales transaction headers', async () => {
  ...
  assert.match(queries[0], /WHERE o\.DelDt = @delDt\s+AND o\.TrTp = 1\s+AND \(ISNULL\(o\.OrdPrSt, 0\) & 536870912\) = 0\s+AND ISNULL\(o\.DelMt, 0\) NOT IN \(150, 151\)/);
  assert.match(queries[1], /FROM Ord o[\s\S]*INNER JOIN @OrderNos f ON f\.OrdNo = o\.OrdNo[\s\S]*WHERE o\.TrTp = 1\s+AND \(ISNULL\(o\.OrdPrSt, 0\) & 536870912\) = 0\s+AND ISNULL\(o\.DelMt, 0\) NOT IN \(150, 151\);/);
